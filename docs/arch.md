...
|---|---|
| src/model_manager.py | Thin, non-duplicating Ollama API wrapper: list, exists, show, pull, delete, ensure_available (existence check before any pull — the single source of truth for "does this model exist locally?"), and create_from_modelfile (the one non-HTTP path: writes a temporary Modelfile and runs the Ollama CLI `ollama create <name> -f <Modelfile>` via `subprocess` — no shell — because the HTTP `/api/create` on this build uses a structured body that cannot express the custom RENDERER/PARSER directives). |
| src/repair_manager.py | Repair orchestration: builds a Modelfile (FROM <orig>, RENDERER <r>, PARSER <p>), creates the derived model (via `ModelManager.create_from_modelfile`, i.e. the Ollama CLI), **verifies** it with a minimal chat probe (exercises the same chat-template path the client will hit), atomically switches the internal name mapping, and deletes the original only **after** the mapping succeeds. Per-model asyncio.Lock prevents duplicate repairs; identical config is idempotent. On startup, reconstructs any prior repair mappings from Ollama stored Modelfile metadata (no separate storage) so a successful repair stays reachable after a restart. |
